...
def tilt_correction(
    ...
    if abs(tilt) < cut_off_angle_deg:
        ...
        return apply_tilt_correction(arrays, tilt, rot_center, ncore)
...
def apply_tilt_correction(
    ...
    if arrays.ndim == 2:
        return rotate(arrays, -tilt, center=rot_center, resize=False, preserve_range=True)
    elif arrays.ndim == 3:
        ...
        with SharedMemoryManager() as smm:
            ...
            rst = process_map(
                partial(rotate, angle=-tilt, center=rot_center, resize=False, preserve_range=True),
